...
function razorSelectedClips(followTargets, excludeOriginal, marginFrames, mode) {
    var seq = app.project.activeSequence;
    if (!seq) { alert("シーケンスを開いてアクティブにしてください。"); return; }

    var selectedClips = seq.getSelection();
    if (selectedClips.length === 0) { alert("クリップを選択してください。"); return; }

    app.enableQE();
    var qeSeq = qe.project.getActiveSequence();
    if (!qeSeq) { alert("QEシーケンスが取得できません。"); return; }

    var frameDuration = framesToSec(seq, 1);
    var fps = Math.round(1 / frameDuration);

    function secsToTimecode(secs) {
        var totalFrames = Math.round(secs / frameDuration);
    ...
    for (var i = 0; i < selectedClips.length; i++) {
        var clip = selectedClips[i];
        if (!clip || !clip.start || !clip.end) continue;
        cutTimecodes[secsToTimecode(clip.start.seconds)] = true;
        cutTimecodes[secsToTimecode(clip.end.seconds)] = true;
    }

    function applyRazors(tracks, getQeTrack) {
        for (var t = 0; t < tracks.numTracks; t++) {
            if (!shouldProcess(tracks[t], followTargets)) continue;
            for (var tc in cutTimecodes) {
                getQeTrack(t).razor(tc);
            }
        }
    }

    applyRazors(seq.videoTracks, function(i) { return qeSeq.getVideoTrackAt(i); });
    applyRazors(seq.audioTracks, function(i) { return qeSeq.getAudioTrackAt(i); });
}
