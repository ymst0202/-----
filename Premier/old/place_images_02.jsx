...
function placeImportedImagesWithFullPathBin() {
    ...
    // 1. 画像名の接頭辞
    var pngFilePrefix = prompt("1/4: 画像名の接頭辞 (Prefix) を入力してください:", "t 2 #2 ");
    if (pngFilePrefix === null) return; // キャンセルされたら終了

    // 2. ビンパス
    var projectBinPath = prompt("2/4: 画像が格納されているビンパスを入力してください:\n（プロジェクト直下の場合は空欄にしてください）", "03_Works/ai_telop");
    if (projectBinPath === null) return;

    // 3. トラック情報 (UIの表示に合わせて「1始まり」で入力させ、内部で-1する)
    var tracksInput = prompt("3/4: 【トラック設定】\n「基準となるビデオトラック番号」と「画像を配置するビデオトラック番号」を半角カンマ区切りで入力してください。\n（例: V2を基準にしてV3に置く場合 → 2,3）", "2,3");
    ...
    // 4. 連番設定
    var numberInput = prompt("4/4: 【連番設定】\n「開始番号」と「桁数」を半角カンマ区切りで入力してください。\n（例: 1から開始でゼロ埋めなし → 1,1 \n 001から開始で3桁 → 1,3）", "1,1");
    ...
    var pngStartNumber = parseInt(numParts[0], 10);
    var pngDigits = parseInt(numParts[1], 10);

    if (isNaN(pngStartNumber) || isNaN(pngDigits)) {
